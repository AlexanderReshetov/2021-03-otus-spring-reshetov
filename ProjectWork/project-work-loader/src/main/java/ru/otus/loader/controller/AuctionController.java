...
public class AuctionController {
    ...
    public ResponseEntity<List<ResponseAuctionDto>> getAllAuctionsByRealmId(@RequestHeader("Authorization") String token, @PathVariable("realmId") Long realmId) {
        return ResponseEntity.ok(loadAuctionsService.getAllAuctionsByRealmId(token, realmId).getBlizzardAuctionDtoList().stream().map(ResponseAuctionDto::toDto).collect(Collectors.toList()));
    }
